...
const QUOTES = [
  ...
  "Reisen ist leben.",
  "Die Erde hat Musik für die, die zuhören.",
  "Abenteuer lohnt sich immer.",
];
...
export default function App() {
  ...
  const onQrScanned = useCallback((loc: PanoramaLocation) => {
    ...
    if (playerCity) {
      setQrBlockedMsg('Diese Stadt ist schon auf dem Tisch!');
  ...
  const captureAndRecognize = useCallback(async () => {
    ...
    try {
      ...
      setScanError('Erfassung fehlgeschlagen — nochmal versuchen');
      setTimeout(() => setScanError(''), 2000);
    }
  ...
  // TUTORIAL
  const TUT_PAGES = [
    { bg: C.bg, titleColor: C.green, title: 'Eine Aufgabe.', body: 'Ein Street View taucht irgendwo auf der Welt auf. Du hältst eine Stadtkarte. Finde heraus welche Stadt auf dem Tisch am nächsten dran ist.\n\nEinfach? Klar. Leicht? Ganz und gar nicht.' },
    { bg: C.blue, titleColor: C.accent, title: 'Ziehen. Scannen. Tickt.', body: 'Zieh eine Karte vom Stapel. Scann den QR-Code mit der App. Ein Street View lädt sofort — und der Timer läuft, ob du bereit bist oder nicht.' },
    { bg: C.bg, titleColor: C.accent, title: 'Nenn die Stadt.', body: 'Studier das Street View. Wähl die nächste Stadt von den Karten auf dem Tisch. Tippe aufs Mikro und sag den Namen — die App merkt sich deine Antwort.\n\nJe näher, desto mehr Punkte.' },
    { bg: '#0a2a0a', titleColor: C.green, title: 'Gefühlte Gefahr?', body: 'Denkst du jemand hat falsch geraten? Setz einen Chip und nenn DEINE Stadt.\n\nRichtig → Bonus-Punkte.\nFalsch → Tschüss, Chip.\n\n→ Los geht\'s!' },
  ];
          ...
          <View style={s.scanOverlay}>
            <View style={{ alignItems: 'center', marginBottom: 20 }}>
              <Text style={{ color: C.primary, fontSize: 13, fontWeight: '700', letterSpacing: 2, marginBottom: 6 }}>
                {showCityScanner ? 'STADT ZUORDNEN' : 'QR KARTE SCANNEN'}
              </Text>
            ...
            <View style={s.scanFrame}>
              <Text style={{ color: C.primary, fontSize: 16, fontWeight: '600', textAlign: 'center' }}>
                {showCityScanner ? 'Stadtkarte ins Bild halten' : 'QR-Karte ins Bild halten'}
              </Text>
            </View>
            ...
            <TouchableOpacity style={s.scanCloseBtn} onPress={() => { setShowCityScanner(false); setShowQrScanner(false); setScanned(false); setManualCode(''); }}>
              <Text style={s.scanCloseText}>SCHLIEẞEN</Text>
            </TouchableOpacity>
          </View>
        </CameraView>
      </View>
        ...
        <View style={{ position: 'absolute', bottom: 40, width: '100%', paddingHorizontal: 30, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          {tutorialPage < TUT_PAGES.length - 1 ? (
            ...
            <TouchableOpacity style={{ backgroundColor: C.green, paddingVertical: 14, paddingHorizontal: 28, borderRadius: 9999 }} onPress={() => setScreen('setup')}>
              <Text style={{ color: C.bg, fontSize: 17, fontWeight: '700', fontFamily: FF.bold }}>Los geht's!</Text>
            </TouchableOpacity>
          )}
        </View>
      </View>
      ...
      <KeyboardAvoidingView style={s.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        <ScrollView contentContainerStyle={s.setupScroll} keyboardShouldPersistTaps="handled">
          ...
          <View style={{ marginTop: 32 }}>
            ...
            <Text style={s.actionHint}>{allPlayersScanned ? `${players.length} Spieler bereit` : 'Stadtkarten für alle Spieler scannen'}</Text>
          </View>
        </ScrollView>
      ...
      return (
        <View style={s.container}><StatusBar hidden />
          ...
          <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 32, paddingTop: 90, paddingBottom: 40 }}>
            ...
            <Text style={{ color: 'rgba(225,224,251,0.5)', fontSize: 14, textAlign: 'center', marginBottom: 32 }}>QR-Karte scannen um den Ort zu sehen</Text>
      ...
      <View style={s.container}><StatusBar hidden />
        ...
        {phase === 'view' && (
          <>
            ...
            {!svLoaded && !svError && <View style={s.loadingOverlay}>
              <Text style={{ color: 'rgba(225,224,251,0.5)', marginBottom: svLoadingLong ? 20 : 0 }}>Street View lädt...</Text>
            ...
            {svError && <View style={s.errorOverlay}>
              <Text style={{ color: C.error, fontSize: 16, marginBottom: 20 }}>Kein Street View</Text>
              <View style={{ gap: 12, width: '100%', alignItems: 'center' }}>
                <TouchableOpacity style={[s.primaryBtn, { width: '80%' }]} onPress={loadNewCard}><Text style={s.primaryBtnText}>NEUE KARTE LADEN</Text></TouchableOpacity>
            ...
            {svLoaded && <>
              ...
              <TouchableOpacity style={s.pickBtn} onPress={() => { playClickSound(); setTimerPaused(true); setPhase('pick'); }}><Text style={s.pickBtnText}>ICH WEIß ES!</Text></TouchableOpacity>
            </>}
          </>
        )}
        ...
        {phase === 'pick' && (
          <View style={s.pickScreen}>
            <Text style={{ color: C.onSurface, fontSize: 20, fontWeight: '700', textAlign: 'center', marginBottom: 4 }}>
              {challengerId !== null ? 'HERAUSFORDERER WÄHLT' : 'WELCHE STADT IST AM NÄCHSTEN?'}
            </Text>
            <Text style={{ color: 'rgba(225,224,251,0.5)', fontSize: 13, textAlign: 'center', marginBottom: 24 }}>
              {challengerId !== null ? `${players.find(p => p.id === challengerId)?.name}, wähl deine Stadt` : `${activePlayer.name}, wähl die Stadt die am nächsten ist`}
            </Text>
        ...
        {phase === 'challenge' && activePickIdx !== null && (
          <View style={s.pickScreen}>
            ...
            <Text style={{ color: C.onSurface, fontSize: 16, textAlign: 'center', marginBottom: 6 }}>{activePlayer.name} hat gewählt:</Text>
          ...
          <View style={s.resultOverlay}>
            <Animated.View style={[s.resultCard, { transform: [{ scale: resultScale }] }]}>
              <TouchableOpacity style={s.primaryBtn} onPress={nextTurn}>
                <Text style={s.primaryBtnText}>{round >= maxRounds ? 'ERGEBNISSE' : 'NÄCHSTE RUNDE →'}</Text>
              </TouchableOpacity>
              {winnerId !== null && <Text style={{ color: C.primary, fontSize: 16, fontWeight: '700', textAlign: 'center', marginTop: 16, marginBottom: 8 }}>⭐ {players.find(pp => pp.id === winnerId)?.name} bekommt den Punkt!</Text>}
              ...
              <Text style={{ color: C.onSurface, fontSize: 17, fontWeight: '600', textAlign: 'center', marginBottom: 12 }}>liegt am nächsten bei {tableCities[closestCityIdx].city}</Text>
  ...
  return (
    <View style={s.container}><StatusBar hidden />
      <ScrollView contentContainerStyle={s.endScroll}>
        ...
        <Text style={{ color: 'rgba(225,224,251,0.4)', fontSize: 11, fontWeight: '700', letterSpacing: 3, textTransform: 'uppercase', textAlign: 'center', marginBottom: 40 }}>{isTie ? `${tiePlayers.map(p => p.name).join(' & ')} gleichstand bei ${sorted[0].score} Pkt — Verlängerung!` : 'SPIELDATEN BEREIF FÜR ANALYSE'}</Text>
        {sorted.map((p, i) => (
